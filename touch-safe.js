...
function generateList() {
	...
	var object_list = ['dome','hammer','heel','hen','igloo','leaf', 'lock', 'moose', 'seal', 'safe'];
...
function addSelect() {
	$('.image-checkbox-container img').each(function() {
		if (this.style.border != '') {
			var index = parseInt($(this).prev().val());
...
function showWrong2() {
    navigator.notification.alert(
        ...
        'Rehearse',            // title
        'See Answer'                  // buttonName
  	);
}
function showWrong() {
	var html='<p>Wrong<a data-role="button">Try Again</a><p>'
...
function generateNextCheck() {
	...
	if (checkIndex == 2) {
		...
		var html = "<p>{0}</p><div class=personDiv><img class=clue src=images/person/{1}.jpg /><div data-role='fieldcontain'><form action='#'><div><input type='text' name='password' id='game-password' value='' placeholder='doing what'/></div><button type='submit' name='submit; value='submit' onclick='generateNextSequence()' >Check and Next</button></form><a href='#' data-role='button' onclick='forgetStory()'>I Forget</a></div></div>";
		$('#gamestories').html(String.format(html, curPerson, curPerson));
		checkIndex += 1;
...
function forgetStory() {
	var html="<p>{0}</p><div class=personDiv><img class=clue src=images/person/{1}.jpg /></div><p>is</p>{2}<div class=actionDiv><img class=clue src=images/action/{3}1.jpg /></div><p>a/an</p> <p>{4}</p><div class=objectDiv><img class=clue src=images/object/{5}1.jpg /></div>";
	var cS = gamelist[checkIndex-1];
	$('#hintSpace').html(String.format(html, cS[0], cS[0], cS[1], cS[1], cS[2], cS[2]));
...
function generateNextSequence() {
	...
	if (storyIndex <= sequenceIndex) {
		if (storyIndex == 0) {
			var html="<p>{0}</p><div class=personDiv><img class=clue src=images/person/{1}.jpg /></div><p>is</p>{2}<div class=actionDiv><img class=clue src=images/action/{3}1.jpg /></div><p>a/an</p> <p>{4}</p><div class=objectDiv><img class=clue src=images/object/{5}1.jpg /></div><a href='#' data-role='button' onclick='generateNextSequence();' >Next</a>";
	...
	//generate next check
		...
		if (checkIndex == -1) {
			...
			var html = "<p>{0}</p><div class=personDiv><img class=clue src=images/person/{1}.jpg /></div><div data-role='fieldcontain'><form action='#'><div><input type='text' name='password' id='game-password' value='' placeholder='doing what'/></div><button type='submit' name='submit; value='submit' onclick='generateNextSequence()' >Check and Next</button></form><a href='#' data-role='button' onclick='forgetStory()'>I Forget</a></div>";
			$('#gamestories').html(String.format(html, curPerson, curPerson));
			checkIndex +=1;
